...
'''
MinMaxObserver继承自ObserverBase,其又继承自nn.module，相当于在调用时会自动调用forward
实例化MinMaxObserver的时候,会调用init，
通过register_buffer()登记过的张量：会自动成为模型中的参数，随着模型移动（gpu/cpu）而移动，但是不会随着梯度进行更新。
将qmin和qmax利用bits进行赋值
'''
class MinMaxObserver(ObserverBase):
    ...
    def calculate_qparam(self):
        ...
        zero_point = 0
        # with open('scale.txt','a') as file:
        #     file.write(f'scale={scale}, 1/scale={1/scale},zero_point={zero_point}\n')
        if scale == 0:
